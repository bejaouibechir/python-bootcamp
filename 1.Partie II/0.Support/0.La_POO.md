...
## Héritage multiple et utilisation de super() en Python

## 1. Héritage multiple : principe

L’héritage multiple permet à une classe d’hériter **de plusieurs classes parentes**.  
La classe enfant récupère les méthodes de toutes les classes dont elle hérite.

### Exemple simple

Créer deux comportements :
...
# classe héritant des deux
class Personne(Marcheur, Nageur):
    pass
...
La classe Personne possède automatiquement :

- marcher() venant de Marcheur

- nager() venant de Nageur

Syntaxe de l’héritage multiple :

class ClasseEnfant(Parent1, Parent2)

---

# 2. Ordre de recherche des méthodes (MRO)

Lorsqu’une méthode est appelée, Python cherche la méthode selon un ordre appelé **MRO (Method Resolution Order)**.
...
Le constructeur de A est exécuté **deux fois**.

C’est le **problème du diamant**.

---

# 4. Solution

changer l'ordre de classe d'héritage c prend la place de B dans le premier ordre
...
Chaque constructeur est exécuté **une seule fois**.
...
# Utiliser super() pour appeler une méthode d’un ancêtre
...
La méthode de la classe parent est exécutée après celle de la classe enfant.
...
# 7. Ancienne syntaxe de super()

Avant Python 3, on écrivait :

```python
super(NomClasse, self).methode()
```
...
class Chien(Animal):
    ...
    def parler(self):
        super(Chien, self).parler()
        print("Aboie")
```

Aujourd’hui on utilise simplement :
...
- compatibilité avec l’héritage multiple

- absence d’appels multiples des constructeurs.
