...
def exporter_rapport_complet(chemin: str, stock_service) -> None:
    ...
    with pd.ExcelWriter(chemin, engine="openpyxl") as writer:
        pd.DataFrame([p.to_dict() for p in sorted(produits)]).to_excel(writer, sheet_name="Stock", index=False)
        pd.DataFrame([p.to_dict() for p in alertes]).to_excel(writer, sheet_name="Alertes", index=False)
        pd.DataFrame([{"categorie": c, **vals} for c, vals in stats.items()]).to_excel(writer, sheet_name="Statistiques", index=False)
